...
public class IntDeque {
	...
	public int dequeFront() throws EmptyIntDequeException {
		if (num <= 0) throw new EmptyIntDequeException(); // 덱(deck)이 비어 있음
		int x = que[front++];
		num--;
	...
	// 덱에서 x를 검색하여 index(찾지 못하면 -1)를 반환
	...
	// 덱에서 x를 검색하여 머리부터 몇 번 째인가(찾지 못하면 0)를 반환
	...
	// 덱내의 데이터를 머리→꼬리의 차례로 나타냄
	public void dump() {
		if (num <= 0) System.out.println("덱이 비었습니다.");
		else {
			for (int i = 0; i < num; i++)
